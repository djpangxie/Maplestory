...
def game(cursor, gender):
    ...
    allSprites = pygame.sprite.OrderedUpdates(maps, border, tracker, healthPointBar, boss, monsters, player, effect,
                                              damages, golds, label, npc, portal, reminder, cursor)
    ...
    attack_sound1 = pygame.mixer.Sound('./Sound Effects/DarkImpaleUse.wav')
    attack_sound1.set_volume(0.3)
    attack_sound2 = pygame.mixer.Sound('./Sound Effects/GunSound.wav')
    attack_sound2.set_volume(0.4)
    ...
    while keepGoing:
            ...
            if event.type == pygame.QUIT:
                ...
                if event.key == pygame.K_x:
                    ...
                    if effect.finish() and player.attack_finished():
                        player.attacking()
                        effect.start(player.get_direction(), player.rect.center, gender)
                        if gender == 3:
                            attack_sound2.play()
                        else:
                            attack_sound1.play()
                elif event.key == pygame.K_UP and player.real_rect.collidepoint(portal.rect.center):
                    enter.play()
                    if current_stage < 3:
                        gold_list = false_list[:]
                        play_sound = True
                ...
                cursor.release()
        if counter % 40 == 0 and player.real_rect.colliderect(boss.rect):
            ...
            for index in range(len(monsters)):
                if player.real_rect.colliderect(monsters[index].rect):
        ...
        for index in range(len(golds)):
            if player.real_rect.colliderect(golds[index].rect):
                gold_sound.play()
...
def instructions(cursor):
    ...
    player = Sprites.Player(screen, random.randrange(1, 4))
...
def hallOfFame(winner, gold_collected, new_time, cursor):
    ...
    if winner:
        ...
        if new_time < fastest:
            fastest = int(new_time)
        if gold_collected > most_gold:
            most_gold = int(gold_collected)
        ...
        for event in pygame.event.get():
            if event.type == pygame.QUIT:
                ...
                if menu_button.rect.collidepoint(event.pos):
                    if not menu_button.get_collide():
